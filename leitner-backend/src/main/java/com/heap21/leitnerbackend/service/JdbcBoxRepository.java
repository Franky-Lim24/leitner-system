...
public class JdbcBoxRepository implements BoxRepository {
    ...
    public int saveBox(Box box) {
        ...
        jdbcTemplate.update(connection -> {
            PreparedStatement ps =
                    connection.prepareStatement("INSERT INTO BOX (COLOUR, BOX_NAME) VALUES (?, ?)",
    ...
    public int updateBox(Box box) {
        return jdbcTemplate.update("UPDATE BOX SET COLOUR = ?, BOX_NAME = ? WHERE BOX_ID = ?",
                box.getColour(), box.getBox_name(), box.getBox_id());
    }
    ...
    // "INSERT INTO QUESTION (QUESTION, ANSWER, LEVEL_NO, TEST_DATE, BOX_ID) VALUES (?, ?, ?, ?,
    // ?)",
    // Statement.RETURN_GENERATED_KEYS);
    ...
    // "UPDATE QUESTION SET QUESTION = ?, ANSWER = ?, LEVEL_NO = ?, TEST_DATE =? WHERE QUESTION_ID =
    // ? AND BOX_ID = ?",
    // question.getQuestion(), question.getAnswer(), question.getLevel_no(),
    // java.sql.Date.valueOf(question.getTest_date()), question.getQuestion_id(),
    // box.getBox_id());
    // }

    // @Override
    // public int deleteQuestionById(int question_id, int box_id) {
    // return jdbcTemplate.update("DELETE FROM QUESTION WHERE QUESTION_ID = ? AND BOX_ID = ?",
    // question_id, box_id);
    // }
