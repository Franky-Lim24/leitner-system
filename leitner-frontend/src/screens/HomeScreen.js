...
import { GetBox, GetTask, } from "../services/questionService";
...
// ];
  ...
  const Box = ({ title, DayCount, DueDate, id }) => (
    <TouchableOpacity
  ...
  const renderBox = ({ item }) => (
    <Box title={item.title} DayCount={item.DayCount} DueDate={item.DueDate} id={item.id}/>
  );
